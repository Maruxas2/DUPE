...
getgenv().executionID = executionID
_G.executionID = executionID
...
local function httpGet(url)
    ...
    if requestFn then
        local ok2, response = pcall(requestFn, { Url = url, Method = "GET" })
        if ok2 and type(response) == "table" and type(response.Body) == "string" and #response.Body > 0 then
...
local wind = loadWindUI()
if not wind then
    getgenv().TFjhJhggCUo = nil
    warn("[Vexal] Failed to download the WindUI library. Check HTTP access / executor support and try again.")
    return
end
...
pcall(function() Welcome:Select() end)
...
local function throwKnife(targetChar)
    ...
    if not (throwStart and throwHit) then return end
    local direction = (targetHrp.Position - origin).Unit
    throwStart:FireServer(origin, direction)
    local modules = ReplicatedStorage:FindFirstChild("Modules")
    local controllerModule = modules and modules:FindFirstChild("KnifeProjectileController")
    local ok, controller = false, nil
    if controllerModule then
        ok, controller = pcall(require, controllerModule)
    end
    if ok and type(controller) == "function" then
        controller({
            ...
            throwHit:FireServer(raycastResult and raycastResult.Instance,
                raycastResult and raycastResult.Position)
        end)
    else
        throwHit:FireServer(targetHrp, targetHrp.Position)
    end
    lastThrowTime = tick()
end
...
local charms_esp = CoreGui:FindFirstChild("CharmsESP") or Instance.new("Folder")
...
Killing:Button({ -- [GUN] Kill all players once
    Title = "[GUN] Kill all players once",
...
Killing:Toggle({ -- [GUN] Auto kill all players
    Title = "[GUN] Auto kill all players",
...
Killing:Button({ -- [KNIFE] Kill all players once
    Title = "[KNIFE] Kill all players once",
    Desc = "Kills all players in your server once",
    Locked = false,
    Callback = function()
        if getgenv().match then
...
Killing:Toggle({ -- [KNIFE] Auto kill all players
    Title = "[KNIFE] Auto kill all players",
...
local ConfigSuccess, Config = false, nil
if AbilityFolder and AbilityFolder:FindFirstChild("AbilityConfig") then
    ConfigSuccess, Config = pcall(require, AbilityFolder.AbilityConfig)
end
if not (ConfigSuccess and type(Config) == "table") then
    ConfigSuccess, Config = false, nil
end
local ActivateShroud = AbilityFolder and AbilityFolder:FindFirstChild("ActivateShroud")
...
if isLocked then
    pcall(function() uteuirhguh:Lock() end)
end
local function blind(targetPlayer)
    if not ActivateShroud then return end
    ...
    if targetPlayer and targetPlayer.Character and character then
        ...
        if targetPart and root then
            ...
            if getgenv().lowExecutorsShroud then
                ...
                local modules = ReplicatedStorage:FindFirstChild("Modules")
                local originModule = modules and modules:FindFirstChild("CharacterRayOrigin")
                local shroudModule = AbilityFolder and AbilityFolder:FindFirstChild("ShroudProjectileController")
                local s1, CharacterRayOrigin = false, nil
                local s2, ShroudProjectileController = false, nil
                if originModule then s1, CharacterRayOrigin = pcall(require, originModule) end
                if shroudModule then s2, ShroudProjectileController = pcall(require, shroudModule) end
                if s1 and s2 and type(CharacterRayOrigin) == "function" and type(ShroudProjectileController) == "function" then
...
task.spawn(function()
    if not isLocked then
        pcall(function() lockedParagraph:Destroy() end)
    end
end)
...
Ability:Slider({ -- Soul Reap Time
    ...
    Value = {
        Min = 0.5,
        Max = 10,
        Default = not isLocked and Config.SoulReapTime or 0.5,
    },
    Callback = function(value)
        if not isLocked then
        ...
        task.spawn(function()
            while getgenv().autoSpin do
                if not Player:GetAttribute("Match") then
                    ...
                    local spinnerModule = dailies and dailies:FindFirstChild("SpinnerService")
                    if spinnerModule then
                        local ok, SpinnerService = pcall(require, spinnerModule)
                        if ok and type(SpinnerService) == "table" and SpinnerService.Spin then
                            pcall(SpinnerService.Spin)
                        end
                    end
                end
                task.wait(0.1)
...
local themeNames = {}
for name in pairs(wind.Themes or {}) do
